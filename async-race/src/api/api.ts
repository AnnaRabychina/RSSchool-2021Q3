...
import { carsOnPage, winnersOnPage } from '../options/options';
...
export const getCars = async (page?: number, limit: number = carsOnPage): Promise<IGetData> => {
...
export const createCar = async (body: IBodyCar) => fetch(`${baseUrl}${Path.garage}`, {
  method: 'POST',
  body: JSON.stringify(body),
  headers: {
    'Content-Type': 'application/json'
...
export const deleteCar = async (id: number) => (await fetch(`${baseUrl}${Path.garage}/${id}`, { method: 'DELETE' })).json();

export const getWinners = async (
  page?: number,
  limit: number = winnersOnPage
): Promise<IGetData> => {
