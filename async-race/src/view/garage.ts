...
class PageGarage extends Page {
  ...
  protected createItemCar(name: string, color: string, id: number) {
    let car = `
    <li class="garage-car">
  ...
  async renderCars () {
    ...
    let carsItems: ICar[] = (await getCars(1, 7)).items;
  ...
  async renderGarage(): Promise<void> {
    const cars = await this.renderCars();
    this.page.append(cars);
  }

  render() {
    this.page.append(this.renderOption(), this.createTitle('garage'), this.createPagination(), );
    this.renderGarage();
